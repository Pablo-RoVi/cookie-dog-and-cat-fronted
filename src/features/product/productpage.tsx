...
const ProductPage = () => {
  ...
  const [isConfirmAdminLoggedModalOpen, setIsConfirmAdminLoggedModalOpen] =
    useState<boolean>(false);
  ...
  const deleteProduct = (unique_id) => {
    if (selectedProduct) {
      if (unique_id) {
        Agent.Product.delete(unique_id)
  ...
  return (
    <div className="max-h-screen bg-white">
      <div className="container mx-auto px-4 py-6">
        ...
        {/* Tabla */}
        {TableModule.table({
          headers: userRoleId === 1 ? headersAdmin : headersEmployee,
          data: currentProducts.map((product: Product) => {
                ...
                <div className="flex justify-between items-center ml-4 mr-4">
                  {buttons.EditButton({
                    onClick: () => {
                      ...
                      handleNavigate(`/products/edit-product`, product);
                    },
                  })}
        ...
        {isConfirmationModalOpen && (
          <Modal
            title={`¿Borrar el producto '${selectedProduct.product_name}'?`}
            confirmAction={() => {
              toggleConfirmationModal();
              toggleConfirmAdminLoggedModal();
            }}
        ...
        {isConfirmAdminLoggedModalOpen && (
          <ConfirmAdminLogged
            ...
            confirmAction={() => {
              toggleConfirmAdminLoggedModal();
              deleteProduct(selectedProduct.unique_id);
            }}
        ...
        {userRoleId === 1
          ? buttons.TurquoiseButton({
              text: "Añadir",
              onClick: () => handleNavigate("/products/add-product"),
            })
          : null}
      </div>
    </div>
